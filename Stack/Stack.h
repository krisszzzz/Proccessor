...
//typedef long long ssize_t;  // Remove
...
unsigned __int64 MurmurHash          (const char* test,       size_t obj_size);
...
Quite a complicated looking function, but it is only used to make the stack remember your printer function.
  ...
  @par int IsInvalidStack_elem_type(const stack_elem_type* stack_t)
  ...
  @return - 0 - if stack correct, else the program will exit (Only if defined STACK_USE_ASSERT)
...
Use this function wisely, because if the stack pointer is null, the program will close
  ...
  @return  - its a void-type function, so nothing will be returned
  @par void IsNullptrStack_elem_type(const stack_elem_type* stack_t)
  ...
  @return - its a void-type function, so nothing will be returned
  @par void DtorStack_elem_type(stack_elem_type* stack_t)
...
{                                                                                                           \
    ...
    MAKE_STACK_INVARIANT(                                                                                   \
        elem_type,                                                                                          \
        stack_t->hash == MurmurHash((char*)stack_t, sizeof(stack_##elem_type) - kHashSize),                 \
        kStackHashIsInccorect                                                                               \
    )                                                                                                       \
    )                                                                                                       \
     MAKE_STACK_INVARIANT(                                                                                  \
...
{                                                                                                           \
    ...
    if(!IsEmptyStack_##elem_type(stack_t)) {                                                                \
            ...
            return *extreme_element_address;                                                                \
    } ON_DEBUG_LVL_1(                                                                                       \
     else {                                                                                                 \
            elem_type null_object = {0};                                                                    \
            return null_object;                                                                             \
        }                                                                                                   \
    )                                                                                                       \
                                                                                                            \
